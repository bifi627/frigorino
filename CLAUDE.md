...
- Server state goes through TanStack Query. Household + active-household hooks live as one-hook-per-file under `ClientApp/src/features/households/` and `features/me/activeHousehold/` (mirroring the backend slices). Lists/Inventories still use the older bundled `src/hooks/use*Queries.ts` — being phased out alongside the backend slice migration. Client state uses Zustand. Do not introduce a third state layer (Redux, Context-as-store) for new features.
