...
- Item rows render through the **shared** `SortableList` / `SortableListItem` (drag handle + per-row ⋮ edit/delete menu), opted into the **`dense`** chrome — a bottom hairline divider instead of a per-item card. `items/components/RecipeItemContent.tsx` lays the row out as a flex row: name + italic comment on the left, quantity chip right-aligned on the name's line. `dense` is an opt-in prop (default `false`), so Lists/Inventories keep their card chrome.
