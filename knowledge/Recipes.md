...
- **Cover image** (best-effort, inline): if the parse found an `image`, after the second save the slice re-fetches it via `RecipeImportService.FetchImageAsync` — the **same** SSRF-guarded `HttpClient` as the page (so the IP check + redirect guard + 15 MB cap apply identically), with an `image/*` content-type pre-filter — runs the bytes through `IImageProcessor` (→ WebP full-res + thumbnail) and `recipe.AddAttachment`s it as the recipe's first (cover) attachment → `coverAttachmentId`. **Every** failure mode (blocked IP, non-image, undecodable bytes, save error) is swallowed: a missing cover never fails the import. No inline blob compensation — orphan blobs from a mid-flight throw are swept by `ReclaimOrphanBlobs` (`TryAttachCoverAsync` in `ImportRecipe.cs`).
- **Tests**: integration tests stub the fetch (`StubRecipeImportService`, which returns no `ImageUrl` → the cover branch is skipped, keeping ITs network-free) and cover the create-page flow, typed-name precedence, and frontend URL validation; the real network fetch+parse-success, the size-cap (`page_too_large`), the cover-image parse shapes, and `FetchImageAsync` (content-type reject + size cap) are covered by the **unit** tests (`JsonLdRecipeParserTests`, `RecipeImportUrlTests`, `RecipeImportServiceTests`), not the IT.
