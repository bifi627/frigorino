Feature: Expiry scan API
  ...
  Scenario: A scan dispatches one notification per inventory for an eligible recipient
  ...
  Scenario: Two inventories each produce a separate dispatch
    Given I am opted in to expiry notifications with a registered device
    And an inventory "Fridge" with an item "Milk" expiring in 1 day
    And an inventory "Pantry" with an item "Flour" expiring in 1 day
    When I POST the expiry scan with a valid maintenance key
    Then the API response status is 200
    And exactly 2 notification dispatches exist for me today

  Scenario: A muted inventory is skipped but an unmuted one still notifies
    Given I am opted in to expiry notifications with a registered device
    And an inventory "Fridge" with an item "Milk" expiring in 1 day
    And an inventory "Pantry" with an item "Flour" expiring in 1 day
    And I have muted notifications for inventory "Pantry"
    When I POST the expiry scan with a valid maintenance key
    Then the API response status is 200
    And exactly 1 notification dispatch exists for me today
  ...
  # Two scans fired at once exercise the real (UserId, InventoryId, SentOn) unique index. They may
