...
interface Props {
    /** Live user settings (undefined while loading). The parent keys this card by load state so the
     *  toggle/lead-days seed from server data via useState initializers instead of a reset effect. */
...
export function NotificationsCard({ data }: Props) {
    ...
    const [enabled, setEnabled] = useState(
        () => data?.expiryNotificationsEnabled ?? false,
    );
    const [leadDays, setLeadDays] = useState(() =>
        data ? String(data.expiryLeadDays) : "3",
    );
    const [supported, setSupported] = useState(true);
    // iOS install hint is a deterministic read of the environment — derive it during render rather
    // than mirror it into state via an effect.
    const iosHint = isIosNeedingInstall();
