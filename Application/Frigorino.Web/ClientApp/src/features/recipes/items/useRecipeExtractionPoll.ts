...
// Polls a single just-created recipe item until its quantity arrives (extraction completed) or
// the deadline passes, then patches the items-list cache. `enabled` should mirror the create
...
export const useRecipeExtractionPoll = (
    ...
    const query = useQuery({
        ...getRecipeItemOptions({
            path: { householdId, recipeId, itemId: itemId ?? 0 },
        }),
        enabled: active,
        refetchInterval: (q) => {
