...
namespace Frigorino.Test.Services
{
    public class ListItemServiceSortOrderTests : IDisposable
    {
        ...
        public async Task CreateItemAsync_FirstItem_ShouldGetNewItemSortOrder()
        {
            ...
            Assert.Equal(SortOrderCalculator.GetNewItemSortOrder(), result.SortOrder);
            Assert.False(result.Status); // Should be unchecked by default
        }

        [Fact]
        public async Task CreateItemAsync_MultipleItems_ShouldAllGetSameSortOrder()
        {
            // Arrange
            ...
            // Assert
            var expectedSortOrder = SortOrderCalculator.GetNewItemSortOrder();
            Assert.Equal(expectedSortOrder, item1.SortOrder);
            Assert.Equal(expectedSortOrder, item2.SortOrder);
            Assert.Equal(expectedSortOrder, item3.SortOrder);

            // Verify all items are returned in correct order
            var allItems = await _service.GetItemsByListIdAsync(_testListId, _testUserId);
            var sortedItems = allItems.OrderBy(i => i.SortOrder).ToList();
            
            // All unchecked items should have the same sort order (new items go to top)
            Assert.All(sortedItems, item => Assert.Equal(expectedSortOrder, item.SortOrder));
        }
        ...
        public async Task ToggleItemStatusAsync_UncheckedToChecked_ShouldMoveToCheckedSection()
        {
            ...
            Assert.Equal(SortOrderCalculator.GetCheckedStatusSortOrder(), toggledItem.SortOrder);
        }
        ...
        public async Task ToggleItemStatusAsync_CheckedToUnchecked_ShouldMoveToUncheckedSection()
        {
            ...
            Assert.Equal(SortOrderCalculator.GetUncheckedStatusSortOrder(), toggledItem.SortOrder);
        }

        [Fact]
        public async Task UpdateItemAsync_StatusChange_ShouldUpdateSortOrder()
        {
            // Arrange
            var request = new CreateListItemRequest { Text = "Test item", Quantity = "1" };
            var item = await _service.CreateItemAsync(_testListId, request, _testUserId);
            ...
            Assert.Equal(SortOrderCalculator.GetCheckedStatusSortOrder(), updatedItem.SortOrder);
        ...
        public async Task ReorderItemAsync_MoveToTop_ShouldGetTopSortOrder()
        {
            ...
            dbItem2!.SortOrder = 101_000;
            dbItem3!.SortOrder = 102_000;
            await _dbContext.SaveChangesAsync();

            var reorderRequest = new ReorderItemRequest { AfterItemId = 0 }; // Move to top
            ...
            Assert.Equal(SortOrderCalculator.GetNewItemSortOrder(), reorderedItem.SortOrder);
        }
        ...
        public async Task ReorderItemAsync_MoveBetweenItems_ShouldCalculateMidpoint()
        {
            ...
            var reorderRequest = new ReorderItemRequest { AfterItemId = item1.Id };
